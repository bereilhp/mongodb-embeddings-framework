...
import { generateHuggingFaceEmbedding } from "../services/huggingface.js";
...
router.post("/embed", async (req, res) => {
  try {
    ...
    const embeddings = await generateHuggingFaceEmbedding(texts);
    res.json({ embeddings });
  } catch (error) {
  ...
  try {
    ...
    for (const doc of documents) {
      ...
      if (textChunk && !doc.embeddings) {
        try {
          const embedding = await generateHuggingFaceEmbedding([textChunk]);
